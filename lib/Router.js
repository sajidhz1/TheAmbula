...
Router.route("/channels/:sku", {
    name: "userChannel",
    waitOn : function(){
      return Meteor.subscribe("user-channel", this.params.sku);
    },
    data: function () {
        return { //TODO- return user in a more secured place
           userObject : Meteor.users.find({  _id : this.params.sku }).fetch()
        }
    }
});
