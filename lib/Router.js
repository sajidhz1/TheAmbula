...
Router.route("/channels/:id", {
    ...
    waitOn: function () {
        ...
        return [
            Meteor.subscribe("user-channel", this.params.id),
            Meteor.subscribe("search-videos-by-owner", this.params.id),
            Meteor.subscribe("search-articles-by-owner", this.params.id)
        ];
    },
    data: function () {
        return {
            ...
            articles: Articles.find({owner: this.params.id}).fetch()
        }
    }
});
...
Router.route('/profile', {
    ...
    data: function () {
        return {
            ...
            articles: Articles.find({owner: Meteor.userId()}).fetch()
        }
    }
});
